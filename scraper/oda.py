...
async def _search_oda(session: AsyncSession[bytes], name: str) -> list[dict[str, object]]:
    ...
    return resp.json().get("products", [])  # type: ignore[no-any-return]
...
async def fetch_prices_batch(
    ...
    async def fetch_one(session: AsyncSession[bytes], product: dict[str, object]) -> None:
    ...
    async with AsyncSession(impersonate="chrome120") as session:  # type: ignore[arg-type]
