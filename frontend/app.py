...
if not ssb_df.empty:
    ...
    if before_mask.any():
        ...
        for i in range(ap + 1, len(chain)):
            prev_val = chain.at[i - 1, "index_val"]
            mom_pct = chain.at[i, "mom_pct"]
            chain.at[i, "index_val"] = prev_val * (1 + mom_pct / 100)
        # Backward chain
        for i in range(ap - 1, -1, -1):
            next_val = chain.at[i + 1, "index_val"]
            next_mom_pct = chain.at[i + 1, "mom_pct"]
            chain.at[i, "index_val"] = next_val / (1 + next_mom_pct / 100)
...
if not breakdown_df.empty:
    ...
    if has_mom:
        ...
        st.caption(
            "ℹ️ MoM % will appear once 30 days of price history "
            "are collected. Showing index level vs January 2026 base (100)."
        )
