...
if not ssb_df.empty:
    ...
    anchor_val = (
        float(agg_daily["index_value"].mean())
    ...
    anchor_ts = (
        ...
        if not agg_daily.empty
        else chain["reference_month"].iloc[-1]
    )
    ...
    if before_mask.any():
        ...
        for i in range(ap + 1, len(chain)):
            mom_factor = 1 + chain.at[i, "mom_pct"] / 100
            chain.at[i, "index_val"] = chain.at[i - 1, "index_val"] * mom_factor
        # Backward chain
        for i in range(ap - 1, -1, -1):
            mom_factor = 1 + chain.at[i + 1, "mom_pct"] / 100
            chain.at[i, "index_val"] = chain.at[i + 1, "index_val"] / mom_factor
...
if not breakdown_df.empty:
    ...
    if has_mom:
        ...
        bar_label = breakdown_df["index_value"].apply(format_index)
        bar_colors = [
