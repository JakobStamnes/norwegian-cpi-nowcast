...
if not ssb_df.empty:
    ...
    if before_mask.any():
        ...
        for i in range(ap + 1, len(chain)):
            fwd_factor = 1 + chain.at[i, "mom_pct"] / 100
            chain.at[i, "index_val"] = chain.at[i - 1, "index_val"] * fwd_factor
        # Backward chain
        for i in range(ap - 1, -1, -1):
            bwd_factor = 1 + chain.at[i + 1, "mom_pct"] / 100
            chain.at[i, "index_val"] = chain.at[i + 1, "index_val"] / bwd_factor
...
if not nowcast_history_df.empty and not ssb_df.empty:
    ...
    if not merged.empty:
        ...
        col1, col2, col3 = st.columns(3)
        col1.metric(
            "Nowcast MAE", f"{mae:.2f} pp",
            help="Mean Absolute Error vs SSB actuals"
        )
        col2.metric(
            "Naive MAE (persist)", f"{naive_mae:.2f} pp",
...
if not breakdown_df.empty:
    ...
    if has_mom:
        plot_col = "mom_pct"
        x_title = "MoM % Change"
        bar_label = breakdown_df["mom_pct"].apply(
            lambda v: f"{v:+.2f}%" if pd.notna(v) else "N/A"
        )
        bar_colors = [
            "#DC2626" if v > 0 else "#16A34A"
            for v in breakdown_df["mom_pct"].fillna(0)
        ]
        breakdown_df = breakdown_df.sort_values("mom_pct", ascending=True)
    else:
        # No MoM yet (< 30 days of history) — show absolute index vs base
        ...
        st.caption(
            "ℹ️ MoM % will appear once 30 days of price history are "
            "collected. Showing index level vs January 2026 base (100) for now."
        )
