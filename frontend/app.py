...
if not ssb_df.empty:
    ...
    if before_mask.any():
        ...
        for i in range(ap + 1, len(chain)):
            prev_idx = chain.at[i - 1, "index_val"]
            mom = chain.at[i, "mom_pct"]
            chain.at[i, "index_val"] = prev_idx * (1 + mom / 100)
        # Backward chain
        for i in range(ap - 1, -1, -1):
            next_idx = chain.at[i + 1, "index_val"]
            mom_next = chain.at[i + 1, "mom_pct"]
            chain.at[i, "index_val"] = next_idx / (1 + mom_next / 100)
...
if not nowcast_history_df.empty and not ssb_df.empty:
    ...
    if not merged.empty:
        ...
        col1, col2, col3 = st.columns(3)
        col1.metric(
            "Nowcast MAE", f"{mae:.2f} pp", help="Mean Absolute Error vs SSB actuals"
        )
        col2.metric(
            "Naive MAE (persist)",
            f"{naive_mae:.2f} pp",
